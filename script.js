...
if (eventModal) {
    document.querySelectorAll('.event').forEach(event => {
        event.addEventListener('click', function () {
            ...
            const modalImageSrc = this.getAttribute('data-modal-src') || thumbnailSrc;
            const title = this.querySelector('h3').innerHTML;
            const description = this.querySelector('p').innerHTML;

            eventModalImg.src = modalImageSrc;
            eventModalTitle.innerHTML = title;
            eventModalDescription.innerHTML = description;
